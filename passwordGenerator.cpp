...
{
    std::cout << "Characters Options:" << std::endl;
    std::cout << "lowercase (i.e. abcdef) - LC" << std::endl;
    std::cout << "Uppercase (i.e. ABCDEF) -UC" << std::endl;
    std::cout << "Number (i.e. 1234) -DG" << std::endl;
    std::cout << "Symbols (i.e. @#$%) -SM" << std::endl;
    std::cout << "ALL - Al" << std::endl
              << std::endl;

    std::cout << "You may choose All or any combination of the first 5 options. Type all desired options separated by a space, then hit \"ENTER\"." << std::endl
              << std::endl;
    std::cout << "Option(s): ";
...
std::string generateRandCharSer(std::set<int> *options, int passwordLength)
{
    ...
    while (characterSets.length() != passwordLength)
    {
        ...
        if (result == 2 && (options->find(2) != options->end() || options->find(0) != options->end()))
        {
            characterSets += std::to_string(result);
        }
        if (result == 3 && (options->find(3) != options->end() || options->find(0) != options->end()))
        {
            characterSets += std::to_string(result);
        }
        if (result == 4 && (options->find(4) != options->end() || options->find(0) != options->end()))
        {
            characterSets += std::to_string(result);
        }
    }
...
std::string generatePassword(std::set<int> *options, int passwordLength)
{
    ...
    // generates string with the length of passwordLength where each digit is a random approved character set for the password
    // i.e. 224332 is approved if they chose lowercase, uppercase, and digits

    bool error = true;
    while (error)
    {
        characterSets = generateRandCharSer(options, passwordLength);
        error = false;
